...
app.delete('/personas', (req, res, next) => {
  ...
  connection.query(
    `DELETE FROM datos WHERE id = ${req.query.id}`,
    function(err, results, fields) {
      if (err) {
        console.error(err);
...
app.post('/personas', (req, res, next) => {
  ...
  connection.query(
    'INSERT INTO datos ( nombre, direccion, curp, nacionalidad, numero, ciudad, estado, codigo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
